...
function HeroCard({ card }) {
  ...
  return (
    <div
      className={`relative flex flex-col gap-5 p-6 rounded-2xl bg-white dark:bg-gray-900 border ${card.border} ${card.ring} transition-all duration-200 shadow-sm`}
    >
      {/* Badge + Icon row */}
...
export default function HeroSection() {
  return (
    <section className="relative w-full min-h-[90vh] flex flex-col items-center justify-center px-4 py-16 bg-gray-50 dark:bg-gray-950 overflow-hidden">

      {/* Background grid pattern */}
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-0 bg-[linear-gradient(to_right,#e5e7eb_1px,transparent_1px),linear-gradient(to_bottom,#e5e7eb_1px,transparent_1px)] dark:bg-[linear-gradient(to_right,#1f2937_1px,transparent_1px),linear-gradient(to_bottom,#1f2937_1px,transparent_1px)] bg-[size:40px_40px] opacity-50"
      />
      ...
      {/* ── 3 Hero Cards ── */}
      <div className="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-5 w-full max-w-5xl mt-10">
        {HERO_CARDS.map((card) => (
      ...
      {/* Bottom note */}
      <p className="relative z-10 mt-10 text-xs text-gray-400 dark:text-gray-600 text-center">
        FloodGuard BD · Built for Bangladesh disaster response ·{" "}
